...
type IsuCondition struct {
	...
	ConditionLevel string    `db:"condition_level"` // Enum
...
func getUserIDFromSession(c echo.Context) (string, int, error) { // TODO: CPU
...
func getJIAServiceURL(db *sqlx.DB) string {
	var config Config
...
func getTrend(c echo.Context) error {
	...
	err = db.Select(
		...
		"SELECT c.* FROM `isu_condition` c JOIN ("+
			" SELECT `jia_isu_uuid`, MAX(`timestamp`) AS `last_timestamp`"+
			" FROM `isu_condition`"+
			" GROUP BY `jia_isu_uuid`"+
			" ) `last`"+
			" ON c.jia_isu_uuid = last.jia_isu_uuid AND c.timestamp = last.last_timestamp",
	)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	...
	for _, c := range lastConditionList {
		lastConditionMap[c.JIAIsuUUID] = c
	}

	for _, character := range characterList {
		isuList := []Isu{}
		err = db.Select(&isuList,
			"SELECT * FROM `isu` WHERE `character` = ?", // TODO: N+1
			character.Character,
		)
		if err != nil {
			c.Logger().Errorf("db error: %v", err)
		...
		for _, isu := range isuList {
			...
			isuLastCondition := lastConditionMap[isu.JIAIsuUUID]
...
func postIsuCondition(c echo.Context) error {
	...
	for _, cond := range req {
		...
		// if err != nil {
		// 	c.Logger().Errorf("calculate condition level error: %v", err)
		// 	return c.NoContent(http.StatusInternalServerError)
		// }
